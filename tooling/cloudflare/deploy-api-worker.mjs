...
async function preflightWorkersAuth(workerName) {
    const token = `${process.env.CLOUDFLARE_API_TOKEN ?? ""}`.trim();
    const accountId = `${process.env.CLOUDFLARE_ACCOUNT_ID ?? ""}`.trim();

    if (!token) {
        throw new Error(
            "Missing CLOUDFLARE_API_TOKEN for API worker deployment. Configure CLOUDFLARE_API_TOKEN_WORKERS or CLOUDFLARE_API_TOKEN.",
        );
    }

    if (!accountId) {
        throw new Error("Missing CLOUDFLARE_ACCOUNT_ID for API worker deployment.");
    }
    ...
    throw new Error(
        `Cloudflare Workers auth preflight failed (${code ?? "unknown"}: ${message}). ` +
        "Use a token with Account > Workers Scripts:Edit (and at minimum Workers Scripts:Read). " +
        "Set it as CLOUDFLARE_API_TOKEN_WORKERS in GitHub Actions secrets.",
    );
}
...
async function main() {
    ...
    await preflightWorkersAuth(workerName);
